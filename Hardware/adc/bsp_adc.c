...
*********************************************************************************************************
*/
uint16_t ADC1ConvertedValue[Sample_Num];
uint16_t ADC2ConvertedValue[Sample_Num];
uint16_t ADC3ConvertedValue[Sample_Num];
...
uint16_t g_usTrigValue = 2*1024/3;//触发数值
//uint16_t g_usWaveBuf1[1];
...
static void TIM1_Config(void)
{
    ...
    */
    ...
    TIM_TimeBaseStructure.TIM_Period =  g_SampleFreqTable[TimeBaseId][0];    //g_SampleFreqTable[TimeBaseId][0]ARR自动重装载寄存器周期的值(定时时间）到设置频率后产生个更新或者中断(也是说定时时间到)
...
{ 
	...
	if((ADC3->SR)&0x01)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
    {
		...
		if(g_usTrigCount == 10240)
		{
			...
			g_usTrigTempFlag = ADC3ConvertedValue[1023]; 
		}
		...
		{
			/* 剩余10239表示触发值是ADC3ConvertedValue[0]是触发值，那么上一个点就是10239 */
			g_usTrigTempFlag = ADC3ConvertedValue[1023 - g_usTrigCount]; 
		}
