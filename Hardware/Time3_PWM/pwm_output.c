...
static void TIM4_GPIO_Config(void)//云台控制
{
 ...
 * 调用  ：内部调用
 */
static void TIM5_GPIO_Config(void) //电机控制
{
  GPIO_InitTypeDef GPIO_InitStructure;
...
static void TIM5_Mode_Config(u16 CCR1_Val,u16 CCR2_Val,u16 CCR3_Val,u16 CCR4_Val)
{
  ...
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM5 , ENABLE);  
	...
	/* Time base configuration */		 
  TIM_TimeBaseStructure.TIM_Period = 100;      //8000(125HZ) 100(10KHZ)PWM周期
  TIM_TimeBaseStructure.TIM_Prescaler = 83;	    //设置预分频：即为1MHz 0的时候为84Mhz
  TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;	//设置时钟分频系数：不分频
  TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;  //向上计数模式

  //TIM_TimeBaseStructure.TIM_Period = 8000;  //PWM周期
