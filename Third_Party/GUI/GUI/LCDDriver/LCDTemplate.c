...
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex) {
  ...
  /* Write into hardware ... Adapt to your system */
  {
        LCD_REG = 0x002A;
		LCD_RAM = (479 - x)>>8;
		LCD_RAM = 0x00FF&(479 - x);
		LCD_RAM = (479 - x)>>8;
		LCD_RAM = 0x00FF&(479 - x);

		LCD_REG = 0x002B;	 
		LCD_RAM = y>>8;
		LCD_RAM = 0x00FF&y;                 //设定X坐标
		LCD_RAM = y>>8;
		LCD_RAM = 0x00FF&y ;
  ...
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) 
	  {
		for (; x0 <= x1; x0++) 
	    {
			...
			{ 
					...
					LCD_REG = 0x002A;
					LCD_RAM = (479 - x1)>>8;
					LCD_RAM = 0x00FF&(479 - x1);
					LCD_RAM = (479 - x0)>>8;
					LCD_RAM = 0x00FF&(479 - x0);
